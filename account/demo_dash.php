...
if ($result && $result->num_rows > 0) {
    ...
    $demoTradeBTC = $userData['demo_trade_btc']; // Add this line to fetch demo_trade_btc
    $demoTradeMoney = $userData['demo_trade_money']; // Add this line to fetch demo_trade_money
...
// CoinMarketCap API endpoint for the BTC exchange rate in USD
$apiUrl = 'https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest';
$apiKey = '04ca0d04-72ee-42b6-9da7-7f3d6db655c1'; // Replace this with your actual CoinMarketCap API key
...
$response = makeHttpGetRequest($apiUrl, $headers);

// Parse the JSON response
$data = json_decode($response, true);

// Check if the response is valid and contains the BTC exchange rate
if (isset($data['data']['BTC']['quote']['USD']['price'])) {
    // Extract the BTC exchange rate
    $btcExchangeRateUSD = $data['data']['BTC']['quote']['USD']['price'];

    // Assuming $demoTradeMoney holds the user's money balance
    $demoTradeMoney = $demoTradeMoney;

    // Convert money to BTC using the fetched exchange rate
    $btc = $demoTradeMoney / $btcExchangeRateUSD;

    // Now you have the BTC value stored in the $btc variable
    // You can use this variable wherever you need the BTC value

    // For example, to echo the BTC value with 8 decimal places:
    $formattedBtc = number_format($btc,5);
    
} else {
    echo "Failed to fetch BTC exchange rate.";
}
...
<head>
  ...
  <title>liontFxasset - Dashboard</title>
    ...
    <meta name="apple-mobile-web-app-title" content="liontFxasset - Dashboard">
    
    <meta itemprop="name" content="liontFxasset - Dashboard">
...
.tradingview-widget-container {
  ...
  @media (min-width: 768px) {
    ...
    /* CSS style for the "End Price" column cells */
  .end-price-cell {
    background-color: green;
    color: white; /* Optionally, you can set the text color to white for better visibility */
  }
</style>
  ...
  <img class="logo-dark logo-img" src="logo1.png" alt="logo-dark">
              ...
              <ul class="nk-menu nk-menu-main ui-s2">
                <li class="nk-menu-item">
                  <a href="demo_dash.php" class="nk-menu-link">
                ...
                <li class="nk-menu-item">
                  <a href="demo_dash.php" class="nk-menu-link">
                    <span class="nk-menu-text">Demo Trade</span>
                  </a>
                </li><!-- .nk-menu-item -->
                <li class="nk-menu-item">
                  <a href="#" class="nk-menu-link">
                    <span class="nk-menu-text">DEMO DASHBOARD IS LIMITED!</span>
                  </a>
                </li><!-- .nk-menu-item -->
                ...
                <li class="dropdown user-dropdown order-sm-first">
                  ...
                  <div class="dropdown-menu dropdown-menu-md dropdown-menu-right dropdown-menu-s1 is-light">
                    ...
                    <div class="dropdown-inner user-account-info">
                      <h6 class="overline-title-alt"> Demo trade balance</h6>
                      <div class="user-balance">$<?php echo $demoTradeMoney; ?> <small
                          class="currency currency-usd">USD</small></div>
                      <div class="user-balance-sub">
                  
                      </div>
                     <div class="dropdown-inner user-account-info">
                      <h6 class="overline-title-alt"> Demo trade BTC</h6>
                      <div class="user-balance">$<?php echo $formattedBtc; ?> <small
                          class="currency currency-usd">USD</small></div>
                      <div class="user-balance-sub">
          ...
          <div class="nk-block-head">
            <div class="nk-block-between-md g-3">
              <div class="nk-block-head-content">
                <div class="nk-block-head-sub"><span>Welcome! to Demo Trade</span></div>
                ...
                <div class="nk-block-des">
                  <p>
Quickly review the overview of your demo trade account. Enjoy the experience!</p>
                </div>
        ...
        <div class="card-inner">
          ...
          <div class="nk-iv-wg2">
            ...
            <div class="nk-iv-wg2-title">
              <h6 class="title">Demo Trade Balance<em class="icon ni ni-info"></em></h6>
            </div>
            <div class="nk-iv-wg2-text">
              <div class="nk-iv-wg2-amount">$<?php echo $demoTradeMoney; ?></div>
            </div>
          </div>
        </div>
      </div><!-- .card -->
    ...
    <div class="col-md-6 col-lg-8">
      <div class="nk-wg-card is-s1 card card-bordered">
        <div class="card-inner">
          <div class="nk-iv-wg2">
            <div class="nk-iv-wg2-title">
              <h6 class="title">Demo BTC Balance<em class="icon ni ni-info"></em></h6>
            </div>
            <div class="nk-iv-wg2-text">
              <div class="nk-iv-wg2-amount">
                $<?php echo $formattedBtc; ?>
              </div>
            </div>
          </div>
        </div>
...
function myFunction() {
  ...
  $.ajax({
    type: "POST",
    url: "demobalancecheck.php", // Replace with the actual server-side script to validate balance
    data: { amount: amount },
    cache: false,
    success: function(response) {
      if (response === "success") {
       ...
       var now = new Date();
            var userTime = now.getFullYear() + '-' + (now.getMonth() + 1) + '-' + now.getDate() + ' ' + now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds();
        var dataString = 'amount=' + amount + '&endtime=' + interval + '&symbol=' + symbol + '&buy=' + buy + '&email=' + email + '&balance=' + balance + '&strikerate=' + strikerate + '&endprice=' + endprice + '&type=' + type + '&trade_time=' + userTime;
        ...
        if (amount === '' || interval === '' || symbol === '' || rate === '') {
         ...
         iziToast.error({ message: "Insufficient balance! Please deposit.", position: "topRight" }); // Display iziToast error notification
      }
    },
    error: function(xhr, status, error) {
      console.log(xhr.responseText); // Log the error response
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response === "success") {
        ...
        iziToast.error({ message: "Insufficient balance!", position: "topRight" }); // Display iziToast error notification
      }
    },
    error: function(xhr, status, error) {
      console.log(xhr.responseText); // Log the error response
...
<div id="coinmarketcap-widget"></div>
                      <div class="nk-iv-wg2-cta">
                        <a href="#" class="btn btn-primary btn-lg btn-block">Withdraw not available in demo!
                          Funds</a>
                        <a href="#" class="btn btn-trans btn-block">Deposit Funds not available in demo!</a>
                      </div>
                    </div>
                  </div>
                </div><!-- .card -->
                  ...
                  <div class="nk-refwg-url">
                    <div class="form-control-wrap">
                      ...
                      <input type="text" class="form-control copy-text" id="refUrl"
                        <input type="text" class="form-control copy-text" id="refUrl" value="https://liontFxasset.com/account/register.php?referrer=<?php echo $username; ?>">
...
// Function to format the strike rate
function formatStrikeRate(strikeRate) {
  const formattedRate = (parseFloat(strikeRate) / 100000).toFixed(5);
 ...
 // Function to add the trend indicator based on "Trading Type" to Strike Rate column
  function addTrendIndicatorToStrikeRate() {
    const tableRows = document.querySelectorAll('table tbody tr');
    tableRows.forEach(function (row) {
      const tradingTypeCell = row.querySelector('td:nth-child(9)'); // Assuming Trading Type is the 9th column (adjust if needed)
      const tradingType = tradingTypeCell.textContent.trim().toLowerCase();
      const strikeRateCell = row.querySelector('td:nth-child(5)'); // Assuming Strike Rate is the 5th column (adjust if needed)

      if (tradingType === 'buy') {
        strikeRateCell.classList.add('positivee');
        strikeRateCell.innerHTML += ` &#x25B2;`; // Green arrow up
      } else if (tradingType === 'sell') {
        strikeRateCell.classList.add('negativee');
        strikeRateCell.innerHTML += ` &#x25BC;`; // Red arrow down
      }
    });
  }
...
// Create a mapping object for interval labels
var intervalLabels = {
  "1": "1 min",
  "3": "3 mins",
  "5": "5 mins",
...
fetch('demo-your-php-script.php')
  ...
  .then((data) => {
    ...
    for (let i = data.length - 1; i >= 0; i--) {
      ...
      row.innerHTML = `
        ...
        <td>${formatStrikeRate(record.strike_rate)}</td>
        <td>${record.status === 'Ongoing' ? 'Loading...' : (record.end_price !== null ? `$${formatCurrency(record.end_price)}` : '')}</td>
        <td>${intervalLabels[record.end_time]}</td> <!-- Display interval label here -->
    ...
    // Add trend indicators based on "Trading Type"
    addTrendIndicatorToStrikeRate();
        ...
        <div class="container-xl wide-lg">
          <div class="nk-footer-wrap">
            <div class="nk-footer-copyright">
              © 2023 <a href="https://liontFxasset.com/account">liontFxasset</a>. All rights reserved            </div>
