...
function myFunction2() {
  ...
  var symbol = document.getElementById("symbol").value;
  var rate = document.getElementById("rate").value;
   var endprice = document.getElementById("endprice").value; // Add this line
  ...
  var dataString = 'amount=' + amount + '&endtime=' + endtime + '&symbol=' + symbol + '&buy=' + buy + '&email=' + email + '&balance=' + balance + '&strikerate=' + strikerate + '&endprice=' + endprice; // Add '&endprice=' + endprice
                ...
                <div class="nk-refwg-stats card-inner bg-lighter">
                  <div class="nk-refwg-group g-3">
                    ...
                    <div class="nk-refwg-info g-3">
                      ...
                      <div class="nk-refwg-sub">
                        <div class="title">
                          $<?php echo $totalReferralEarnings; ?></div>
    ...
    <div class="card-body">
      <div class="box-body no-padding">
        <div class="table-responsive">
          <table class="table table-adaptive" style="text-align: center" id="tradeTable">
            <thead>
              <tr>
                <th>Order ID</th>
                <th>Trade Time</th>
...
<script>
  // JavaScript code for fetching and updating trade data
  fetch('your-php-script.php')
    ...
    .then(data => {
      ...
      const formatTradeTime = (timestamp) => {
        const now = Date.now();
        const tradeTimestamp = Date.parse(timestamp);
        const elapsed = now - tradeTimestamp;
      ...
      // Function to update the trade time continuously
      const updateTradeTime = () => {
        const tradeTimeElements = document.querySelectorAll('#tradeTable tbody td.trade-time');
        tradeTimeElements.forEach(element => {
          const timestamp = element.dataset.timestamp;
          const formattedTime = formatTradeTime(timestamp);
          element.textContent = formattedTime;
        });
      };

      // Update trade time initially
      updateTradeTime();

      // Start updating trade time every second
      setInterval(updateTradeTime, 1000);
      ...
      data.forEach(record => {
        ...
        row.innerHTML = `
          ...
          <td class="trade-time" data-timestamp="${record.trade_time}">${formatTradeTime(record.trade_time)}</td>
          <td>${record.symbol}</td>
          <td>${record.amount}</td>
          <td>${record.strike_rate}</td>
          <td>${record.end_price}</td>
