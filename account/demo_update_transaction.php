...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Debugging: Check the value of $updatedEndPrice
    echo "Updated End Price: " . $updatedEndPrice . "<br>";

    // Update the End Price and Status in the 'trades' table for the specific transaction
    $updateQuery = "UPDATE demo_trades SET end_price = '$updatedEndPrice', status = '$updatedStatus' WHERE transaction_id = '$transactionID'";
    ...
    if (mysqli_query($connection, $updateQuery)) {
        ...
        if ($updatedStatus === "Won") {
            ...
            $usernameQuery = "SELECT username FROM demo_trades WHERE transaction_id = '$transactionID'";
            ...
            if ($usernameResult && mysqli_num_rows($usernameResult) > 0) {
                ...
                $updateMoneyQuery = "UPDATE users SET demo_trade_money = demo_trade_money + ? WHERE username = ?";
...
<html>
<head>
    <title>Update Demo Transaction</title>
    <style>
        body {
...
<body>
    ...
    <form action="demo_update_transaction.php?id=<?php echo $_GET['id']; ?>" method="post">
