...
$query = "INSERT INTO demo_trades (transaction_id, symbol, amount, strike_rate, end_time, end_price, status, trading_type, username, trade_time) VALUES ($transactionId, '$symbol', $amount, $strikerate, '$endtime', $endprice, '$status', '$tradingType', '$username', '$tradeTime' )";

if (mysqli_query($conn, $query)) {
  // Update the user's balance in the `users` table
  $updateQuery = "UPDATE users SET demo_trade_money = demo_trade_money - $amount WHERE username = '$username'";
  if (mysqli_query($conn, $updateQuery)) {
    ...
    $subject = 'New Demo Trade Notification';
    $message = "A user has made a demo trade:\n\n";
